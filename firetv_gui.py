...
from scan_lan import scan_lan_com_cancelamento
...
class FireTVGUI:
    ...
    def _build_interface(self):
        ...
        self.ip_frame = tk.Frame(self.root, bg="#1e1e1e")
        ...
        # Dpad frame
        self.dpad_frame = tk.Frame(self.root, bg="#1e1e1e")
        ...
        # Extra buttons
        self.extra_frame = tk.Frame(self.root, bg="#1e1e1e")
        ...
        tk.Button(self.root, text="Enviar Texto", command=lambda: threading.Thread(target=self.enviar_texto, daemon=True).start()).pack()

        tk.Button(self.root, text="Capturar Tela", command=lambda: threading.Thread(target=self.capturar_tela, daemon=True).start()).pack(pady=5)

        self.status_text = tk.Text(self.root, height=6, width=60, state='disabled', bg="#2d2d2d", fg="white")
    ...
    def varrer_rede(self):
        ...
        self.btn_cancelar.pack(side=tk.LEFT, padx=5)
        threading.Thread(target=self._scan_thread, args=(base_ip,), daemon=True).start()
    ...
    def _scan_thread(self, base_ip):
        def progresso_callback(progresso, tempo_restante_str):
            self.root.after(0, lambda: self.progress.config(value=progresso))
            self.root.after(0, lambda: self.tempo_restante_label.config(text=tempo_restante_str))
        encontrados = scan_lan_com_cancelamento(base_ip, FIRE_TV_PORT, self.cancelar_varredura, self.atualizar_status, progresso_callback)
